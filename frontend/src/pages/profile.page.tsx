...
const ConfigForm: React.FC<ConfigFormProps> = ({ videos }) => {
  ...
  async function handleConfirm(formData: any) {
    ...
    else maxTrainingFrames = -1;
    let baseVideoId;
    if (formData.enableTransfer && formData.baseVideoId)
      baseVideoId = formData.baseVideoId;
    else baseVideoId = -1;
    return userService
      .newTrackJob(
        videoMeta.id,
        ...
        maxTrainingFrames,
        baseVideoId,
        formData.enableStage1,
        formData.enableStage2
      )
      .then((res) => {
        message.success(res.data.message);
        requestJobs();
    ...
    >
      ...
      >
        ...
        <ProForm.Group>
          ...
          />
          <ProFormDependency name={["enableTransfer"]}>
            {({ enableTransfer }) => {
              return (
                <ProFormSelect
                  name="baseVideoId"
                  label="Select a base video"
