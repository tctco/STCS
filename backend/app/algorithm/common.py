...
def create_logger(name: str, file_path: str, level=logging.DEBUG) -> logging.Logger:
...
class InstanceGroup:
    ...
    def get_datum(self, video_id, frame_idx, ids, scale=1, track_id=None):
        for mask, bbox, bbox_score, kpts, kpt_scores, centroid, area, id in zip(
            ...
            kpt_scores_str = str(list(map(lambda x: round(x, 2), kpt_scores.flatten())))
...
def crop_min_area_rect(
    img, rect, pose, ismask: bool, upper_kpt_index: int = 0, lower_kpt_index: int = 1
):
    ...
    if (
        rotated_pose[0, 1] >= 0
        and rotated_pose[upper_kpt_index, 1] > rotated_pose[lower_kpt_index, 1]
    ):
...
def set_job_meta(key: str, value):
    """set job meta data to current rq job

    Args:
        key (str): key of the meta data
        value (Any): value of the meta data (must be pickleable)
    """
    job = get_current_job()
    if job:
        job.meta[key] = value
