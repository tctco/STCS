...
_video = session.query(Video).filter(Video.id == VIDEO_ID).first()
resource_path = os.environ.get("RESOURCE_PATH", "static")
file_save_path = Path(
    f"{resource_path}/exp/{_video.user_id}/{_video.name.split('.')[0]}_{_video.original_name.split('.')[0]}"
)
logger = create_logger("stage2", str(file_save_path / "stage2.log"))
__video = mmcv.VideoReader(VIDEO_PATH)
SOFT_BORDER = round(__video.fps * 0.5)
...
class Track:
    ...
    def __repr__(self) -> str:
        return f"{self.id}: framecnt: {self.frame_cnt}, merged: {sorted(self.merged)}, conf: {self.conf}, cls: {self.assigned_class}"
    ...
    def merge(self, track: "Track", ignore_overlap=False):
        ...
        self.distance = self.distance + track.distance
        self.speed = self.distance / self.frame_cnt
...
class TrackMerger:
    def __init__(self, tracks: List[Track], max_det) -> None:
        ...
        pics_root = file_save_path / "cropped"
        img_shapes = []
        self.filter_tracks(min_conf=MIN_CLS_CONF)
        for t in self.tracks:
            for fname in os.listdir(pics_root / str(t.original_id))[::5]:
                fpath = pics_root / str(t.original_id) / fname
    ...
    def build_cfg(self, config_file_path):
        ...
        work_dir = osp.join(
            f"./exp/cls_models/cls_{VIDEO_NAME}_work_dirs",
            osp.splitext(osp.basename(CLS_CONFIG_FILE))[0],
        )
        cfg.work_dir = work_dir
        cfg.train_dataloader.dataset.dataset.data_root = str(file_save_path / "cropped")
        cfg.val_dataloader.dataset.data_root = str(file_save_path / "cropped")
        cfg.test_dataloader.dataset.data_root = str(file_save_path / "cropped")
        # cfg.train_dataloader.dataset.dataset.data_root = (
        #     f"./exp/files/{VIDEO_NAME}/cropped/"
        # )
        # cfg.val_dataloader.dataset.data_root = f"./exp/files/{VIDEO_NAME}/cropped/"
        # cfg.test_dataloader.dataset.data_root = f"./exp/files/{VIDEO_NAME}/cropped/"
        cfg.train_dataloader.batch_size = BATCH_SIZE
        cfg.train_dataloader.dataset.oversample_thr = 1 / MAX_DET
        cfg.val_dataloader.batch_size = BATCH_SIZE
        cfg.test_dataloader.batch_size = BATCH_SIZE
        cfg.auto_scale_lr.base_batch_size = BATCH_SIZE
        cfg.train_dataloader.dataset.dataset.pipeline[1]["crop_size"] = self.img_shape
        ...
        if self.scale_factor < 1:
            cfg.train_dataloader.dataset.dataset.pipeline.insert(
    ...
    def update_cls_model(self, total_train=1000) -> dict:
        ...
        print("pwd is", os.getcwd(), CLS_CONFIG_FILE)
        cfg = self.build_cfg(CLS_CONFIG_FILE)

        best_cls_model_dir = Path(
            f"./exp/cls_models/cls_{VIDEO_NAME}_work_dirs/osnet_x0_25"
        )
        best_model = None
        if (best_cls_model_dir / "best_accuracy.pth").exists():
            cfg.load_from = f"exp/cls_models/cls_{VIDEO_NAME}_work_dirs/osnet_x0_25/best_accuracy.pth"
        else:
        ...
        if self.first_train:
            ...
            patience = 3 * round(1 + self.new_imgs_cnt * 5 / self.old_imgs_cnt)
        else:
            patience = 5 * round(1 + self.new_imgs_cnt * 2.5 / self.old_imgs_cnt)
        patience = min(patience, 20)
        cfg.custom_hooks[0]["patience"] = patience
    ...
    def merge(self, track_ids: set):
        ...
        logger.debug(f"merging group {track_ids}")
    ...
    def filter_tracks(self, min_len=10, min_conf=0.6, export_difficult_frames=True):
        ...
        if len(mean_areas) <= MAX_DET * 2:
            logger.info("too few tracks, skip track filtering")
            return
        ...
        logger.info(
            f"{len(too_large_area)} tracks with too large mean area ({too_large_area}), {len(too_small_area)} tracks with too small mean area ({too_small_area})"
        )
        # speeds = []
        ...
        too_short = list(np.arange(len(self.tracks))[mean_weights <= min_len])
        speed_threshold = mean_speed - 3 * std_speed
        too_slow = list(np.arange(len(self.tracks))[speeds < speed_threshold])
        not_confident = list(np.arange(len(self.tracks))[mean_conf < min_conf])
        # too_slow = [t.id for t in self.tracks if t.speed < speed_threshold]

        logger.info(
            f"{len(too_short)} tracks are too short , {len(not_confident)} tracks are not confident, {len(too_slow)} tracks are too slow ({speed_threshold}, {too_slow}), {len(too_large_area)} tracks have too large mean area ({area_threshold}), {len(too_small_area)} tracks have too small mean area ({area_threshold_low})"
        )
        self._trash = list(
            set(too_short + not_confident + too_large_area + too_slow + too_small_area)
        )
        logger.info(f"Total # of discarded tracks: {len(self._trash)}")
    ...
    def export_difficult_frames(self, original_track_ids):
        ...
        difficult_frames = []
        target_dir = file_save_path / "difficult_frames"
        logger.info(
            f"Exporting difficult frames... You may need to annotate these frames for better performance. The images are saved to {str(file_save_path / 'difficult_frames')}"
        )
        if osp.exists(target_dir):
            shutil.rmtree(target_dir)
        target_dir.mkdir()
        img_root = file_save_path / "cropped"
    ...
    def update_tracks_groups(self, discard=False):
        ...
        for c in cliques:
            ...
            distance = min([self.tracks[id].distance for id in c])
            result.append({"ids": c, "lifespan": lifespan, "distance": distance})
        self.track_groups = sorted(
            result,
            key=lambda x: (len(x["ids"]), x["lifespan"], x["distance"]),
            reverse=True,
        )
        if discard:
    ...
    def plot(self, title="", output_file=None):
        if self.plot_cnt == 0:
            try:
                shutil.rmtree(file_save_path / "plots")
            except:
                pass
            Path(file_save_path / "plots").mkdir(parents=True, exist_ok=True)
        ...
        # Path(f"./exp/files/{VIDEO_NAME}/plots").mkdir(parents=True, exist_ok=True)
        Path(file_save_path / "plots").mkdir(parents=True, exist_ok=True)
        plt.savefig(
            str(file_save_path / f"plots/track_{self.plot_cnt}.png"),
            # f"./exp/files/{VIDEO_NAME}/plots/tracks_{self.plot_cnt}.png",
            bbox_inches="tight",
            dpi=300,
        )
        self.plot_cnt += 1
    ...
    def export_dataset(self, stride=MIN_STRIDE, split_train_val=0.1):
        ...
        if not self.dataset_exported:
            ...
            for g in self.track_groups:
                ...
                if None in assigned_classes:
                    ...
                    break
        # root = Path(f"./exp/files/{VIDEO_NAME}/cropped")
        root = file_save_path / "cropped"
        ...
        for i in range(self.max_det):
            ...
            if len(data_lists["train"][i]) > 4800:
                data_lists["train"][i] = random.sample(data_lists["train"][i], 4800)
            if len(data_lists["val"][i]) > 1200:
                data_lists["val"][i] = random.sample(
                    data_lists["val"][i], min(1200, min_val_len)
                )
            if len(data_lists["train"][i]) < BATCH_SIZE:
                data_lists["train"][i].extend(
                    random.choices(
                        data_lists["train"][i],
                        k=BATCH_SIZE - len(data_lists["train"][i]),
                    )
                )
    ...
    def export_triplet_dataset(self):
        from collections import defaultdict

        contradictory_map = defaultdict(set)
        for g in self.track_groups:
            extended_ids = [self.tracks[id].merged for id in g["ids"]]
            if len(extended_ids) < 2:
                continue
            for i in range(len(extended_ids)):
                for j in range(i + 1, len(extended_ids)):
                    for id1 in extended_ids[i]:
                        for id2 in extended_ids[j]:
                            contradictory_map[id1].add(id2)
                            contradictory_map[id2].add(id1)
        # root = Path(f"./exp/files/{VIDEO_NAME}/cropped")
        root = file_save_path / "cropped"
        frame_cnts = np.array([self.tracks[id].frame_cnt for id in contradictory_map])
        classes = [str(id) for id in contradictory_map]
        data_list = []
        for id in contradictory_map:
            fnames = sorted(
                os.listdir(root / str(id)),
                key=lambda x: int(x.split(".")[0]),
            )
            if len(fnames > 5000):
                fnames = random.sample(fnames, 5000)
            fnames = [f"{str(id)}/{img}" for img in fnames]
            for fname in fnames:
                data_list.append({"img_path": fname, "gt_label": id})
        data = {
            "metainfo": {
                "classes": classes,
                "contradictory_tracklets": contradictory_map,
            },
            "data_list": data_list,
        }
        return data
    ...
    def _batch_inference(self, filepaths):
        ...
        test_pipeline_cfg = self.build_cfg(
            CLS_CONFIG_FILE
    ...
    def get_classification_scores(self, stride: int) -> np.ndarray:
        ...
        if self.cls_model_updated:
            for t in tqdm(self.tracks):
                extended_ids = t.merged
                paths = []
                for id in extended_ids:
                    # img_root = Path(f"./exp/files/{VIDEO_NAME}/cropped") / str(id)
                    img_root = file_save_path / "cropped" / str(id)
                    fnames = os.listdir(img_root)
                    if len(fnames) < 32:
                        _stride = 1
                    else:
                        _stride = stride
                    paths.extend([str(img_root / fname) for fname in fnames[::_stride]])
                    # result = self._batch_inference(paths)
                    # scores[t.id].append(result)
                if len(paths) > 5000:
                    paths = random.sample(paths, 5000)
                scores[t.id] = np.mean(self._batch_inference(paths), axis=0)
    ...
    def force_merge(self):
        ...
        assigned_classes = [self.tracks[id].assigned_class for id in ids]
        assert (
            None not in assigned_classes
        ), f"assigned classes: {assigned_classes}"  # TODO: something wrong here
        target = ids
        ...
        for g in self.track_groups:
            ...
            matches = linear_assignment(dist_mat)
            for i, j in matches:
                if dist_mat[i, j] > 0.5:  # DEBUG 消除质量太差的匹配
    ...
    def classify_uncertain_tracks(
        ...
        if classified_frames / total_frames > ACCUMULATED_FRAME_RATIO:
            logger.info(
                f"{ACCUMULATED_FRAME_RATIO*100}% of frames are classified, skip classification and start merging..."
            )
            self.force_merge()
            return True
    ...
    def differentiate(self):
        ...
        if len(to_be_merged) == 0 and best_match is not None:
            APPEARANCE_THRESHOLD = APPEARANCE_THRESHOLD * 1.5
            logger.warning(
                f"no good match found, the lowest cosine dist is {min_dist}. We are now more tolerant to unclassified tracks by increasing APPEARANCE_THRESHOLD 50% to {APPEARANCE_THRESHOLD}"
            )
        ...
        if accumulated_frames > MIN_MERGE_FRAMES and APPEARANCE_THRESHOLD >= 0.05 * 1.5:
            APPEARANCE_THRESHOLD = APPEARANCE_THRESHOLD / 1.5
            logger.info(f"resetting APPEARANCE_THRESHOLD to {APPEARANCE_THRESHOLD}")
        for s in clean_sets:
            self.merge(s)
    ...
    def remove_contradict(self, l_sets: list) -> list[set]:
        l_sets = [list(s) for s in l_sets]
        for s in l_sets:
            s = sorted(s, key=lambda x: self.tracks[x].distance, reverse=True)
            for id in s[::-1]:
                intersection = set(s) & self.contradictory_tracks[id]
                if len(intersection) > 1:
                    s.remove(id)
        l_sets = [set(s) for s in l_sets]
        return l_sets
    ...
    def output_id_mapper(self):
        ...
        for t in self.tracks:
            ...
            for id in t.merged:
                id_mapper[id] = label
        with open(f"./exp/files/{VIDEO_NAME}/idmapper.pkl", "wb") as f:
    ...
    def save_state(self):
        ...
        self.cls_model = None
        p = Path(f"./exp/files/{VIDEO_NAME}/backup")
    ...
    def restore(backup_id) -> "TrackMerger":
        ...
        with open(
            f"./exp/files/{VIDEO_NAME}/backup/backup_{backup_id:0>6}.pkl", "rb"
        ) as f:
...
if __name__ == "__main__":
    ...
    if resume > 0:
        ...
        session = Session()
        logger.info(f"video is: {_video.original_name} (id: {VIDEO_ID})")
        tracks_info = (
    ...
    while not fixed_tracks:
        ...
        # with open(f"./exp/files/{VIDEO_NAME}/cropped/train.json", "w") as f:
        with open(file_save_path / "cropped/train.json", "w") as f:
            json.dump(train, f)
        # with open(f"./exp/files/{VIDEO_NAME}/cropped/val.json", "w") as f:
        with open(file_save_path / "cropped/val.json", "w") as f:
            json.dump(val, f)
        ...
        while (
            ...
            if classified:
                try:
                    while 1:
                        track_merger.update_contradictory_tracks()
                        not_discarded = track_merger.update_tracks_groups(discard=True)
                        if not not_discarded:
                            continue
                        track_merger.merge_confirmed_tracks()
                        break
                    ...
                    track_merger.cls_model = cls_model
                    track_merger.classify_uncertain_tracks(1, MIN_STRIDE)
                    classified = False
                    break
            else:
                # break
    ...
    id_mapper = track_merger.output_id_mapper()
