...
def is_best_model(model_path: str) -> bool:
    """check if model is best model by checking if it is a .pth file and contains "best" and "epoch"
...
class MMPretrainClassifier:
    ...
    def train(self, patience: int):
        ...
        for model in os.listdir(self.work_dir):
            if is_best_model(model):
                ...
                break
        if best_model:
            shutil.move(self.work_dir / best_model, self.work_dir / "best_accuracy.pth")
    ...
    def _build_config(
        ...
        config.train_dataloader.dataset.dataset.data_root = self.img_root
        config.val_dataloader.dataset.data_root = self.img_root
        config.test_dataloader.dataset.data_root = self.img_root
        ...
        config.val_dataloader.dataset.pipeline[1]["crop_size"] = img_shape
        config.test_dataloader.dataset.pipeline[1]["crop_size"] = img_shape
        if scale < 1:
